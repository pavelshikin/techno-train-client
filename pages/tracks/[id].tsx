        ...
        <div className="container">
           ...
           <div className={s.content}>
              ...
              <div>
                 <h1>Комментарии</h1>
                 <div className="my-1">
                    <TextField {...username} label="Ваше имя" fullWidth variant="outlined"/>
                 </div>
                 <div className="my-1">
                    <TextField {...text} label="Комментарий" fullWidth multiline rows={4} variant="outlined"/>
                 </div>
                 <div className="my-1">
                    <Button onClick={addComment} color="primary" variant="contained">Отправить</Button>
                 </div>
              </div>
              <div className="mt-2">
                 {track.comments.map(comment =>
                   <div className={s.comment} key={comment._id}>
                      <div className={s.commentAuthor}>Автор - {comment.username}</div>
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
   ...
   if (token) {
      const res = await api.get('http://localhost:5000/tracks/' + ctx.params.id, {
         headers: {
